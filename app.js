...
function evolutionSvg(evolution, options = {}) {
  ...
  const points = plotPositions
    .map((point, index) => {
      ...
      const checkpointLabel = dense
        ? `<text x="${x - 4}" y="${height - 46}" text-anchor="end" transform="rotate(-28 ${x - 4} ${height - 46})" class="axis checkpoint-label">${esc(point.place)}</text>`
        : `<text x="${x}" y="${height - 34}" text-anchor="middle" class="axis checkpoint-label">${esc(point.place)}</text>`;
...
function splitTimesTable(evolution, officialTimeEvolution = null, race = null) {
  ...
  const note = `<p class="split-note">Official-time rank is based on elapsed race time, not wall-clock timing order, so different start times do not matter. Start is not ranked. The Mål row uses the same comparison field as your result rank. Overtake is the segment pass balance from the timing mats.</p>`;
  const headers = [
    ...
    helpHeader("Leg", "Elapsed time since the previous timing mat."),
    helpHeader("Official-time rank", "Rank by elapsed race time at this checkpoint among finishers in the same comparison field. Start is not ranked; the Mål row should match your result rank."),
    helpHeader("Δ rank", "Change in official-time rank since the previous timing mat. Green + means you gained places; red - means you lost places."),
    helpHeader("Overtake", "Segment pass balance from the timing mats. Green + means you overtook more riders than passed you; red - means more riders passed you."),
  ].join("");
  return `<div class="table-scroll"><table class="data-table split-time-table"><thead><tr>${headers}</tr></thead><tbody>${evolution.positions
    .map((point, index) => {
      const previous = evolution.positions[index - 1];
      const official = officialByCheckpoint.get(point.checkpointIndex);
      const previousOfficial = previous ? officialByCheckpoint.get(previous.checkpointIndex) : null;
      const leg = previous ? duration(point.elapsedMinutes - previous.elapsedMinutes) : "-";
      const officialDelta =
        official && previousOfficial && Number.isFinite(official.position) && Number.isFinite(previousOfficial.position)
          ...
          : null;
      const segment = previous ? segmentByLabel.get(`${previous.place} -> ${point.place}`) : null;
      ...
      return `<tr><td>${esc(point.place)}</td><td>${kms[point.checkpointIndex] ?? `${point.checkpointIndex + 1}/${evolution.positions.length}`}</td><td>${esc(point.elapsed)}</td><td>${leg}</td><td>${officialText}</td><td>${signedValueHtml(officialDelta, index, { invert: true })}</td><td>${signedValueHtml(directNet, index)}</td></tr>`;
    })
...
function checkpointKms(race) {
  const exact = {
    "vatternrundan-2026": [0, 48, 104, 168, 174, 207, 242, 281, 313, 315],
...
function splitSeries(race, rider) {
  ...
  return splits.map((split, index) => {
    ...
    const legMinutes = previous ? split.elapsedMinutes - previous.elapsedMinutes : split.elapsedMinutes;
    ...
    return {
      ...
      leg: legMinutes,
      speed: legMinutes > 0 && legKm > 0 ? legKm / (legMinutes / 60) : null,
...
function simpleBarChartSvg(rows, options = {}) {
  ...
  const bars = rows
    .map((row, index) => {
      ...
      return `<rect x="${x}" y="${y}" width="${barWidth}" height="${top + chartHeight - y}" fill="${row.color || "#99e9f5"}"><title>${esc(row.label)}: ${row.value.toLocaleString()}</title></rect><text x="${x + barWidth / 2}" y="${labelY}" text-anchor="end" transform="rotate(-34 ${x + barWidth / 2} ${labelY})" class="axis">${esc(row.label)}</text>`;
    })
    .join("");
...
function barLineChartSvg(rows) {
  ...
  const bars = rows
    .map((row, index) => {
      ...
      const labelY = height - 82;
      return `<rect x="${x}" y="${y}" width="${barWidth}" height="${top + chartHeight - y}" fill="#99e9f5"><title>${esc(row.label)}: ${row.count} DNF, ${row.rate.toFixed(1)}%</title></rect><text x="${x + barWidth / 2}" y="${labelY}" text-anchor="end" transform="rotate(-34 ${x + barWidth / 2} ${labelY})" class="axis">${esc(row.label)}</text>`;
    })
    .join("");
  const line = rows.map((row, index) => `${index === 0 ? "M" : "L"} ${xFor(index)} ${yRate(row.rate)}`).join(" ");
  const dots = rows.map((row, index) => `<circle cx="${xFor(index)}" cy="${yRate(row.rate)}" r="4" fill="#ff4f8b" />`).join("");
  return `<svg class="chart dnf-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="DNF by start group">${bars}<path d="${line}" fill="none" stroke="#ff4f8b" stroke-width="3" />${dots}<line x1="${left}" x2="${width - right}" y1="${top + chartHeight}" y2="${top + chartHeight}" class="axis-line" /><text x="${left}" y="24" class="axis">DNF count</text><text x="${width - right}" y="24" text-anchor="end" class="axis">DNF rate %</text></svg>`;
}
...
function dnfDashboardPanel(race) {
  ...
  const earlySpeedSvg = lineChartSvg("Early speed: DNF vs finishers", "km/h", [
    { label: "DNF", color: "#ff4f8b", points: speedRows.map((row) => ({ label: row.label, value: row.dnf })) },
    { label: "Finishers", color: "#22c55e", points: speedRows.map((row) => ({ label: row.label, value: row.finishers })) },
...
function comparePanel(race, selected) {
  ...
  const colors = ["#23d3ee", "#ff4f8b", "#111827", "#22c55e", "#d97706"];
    ...
    .map((checkpoint, index) => {
      const cells = riders
        .map((rider) => {
          ...
          return `<td>${point ? `${hhmm(point.elapsed)} ${point.speed ? `<span>${point.speed.toFixed(1)}</span>` : ""}` : "-"}</td>`;
        })
        .join("");
...
function nationalityChart(race) {
  ...
  const palette = ["#27c7df", "#ff4f8b", "#1fc66a", "#ffc233", "#7c5cff", "#ff8a33", "#19b6a6", "#8bc34a", "#68a8ff", "#f85f73"];
  const grid = [0, 0.25, 0.5, 0.75, 1]
    .map((tick) => {
...
function renderSelected(race, rider) {
  ...
  const officialTimeEvolutionPanel = `
    <div class="panel">
      <div class="panel-heading"><div><h3>Official-time checkpoint rank</h3><p>elapsed-time ranking at each timing mat, regardless of start time</p></div></div>
      ${evolutionSvg(officialTimeEvolution, {
        label: "Official-time checkpoint rank",
        noteTitle: "Official-time rank",
        note: "This chart ranks riders by elapsed race time at each timing mat within the same comparison field as your result. The Mål point should match your result rank.",
        large: true,
      })}
    </div>
  `;
  ...
  const splitPanel = `
    <div class="panel compact-panel">
      ...
      ${splitTimesTable(evolution, officialTimeEvolution, race)}
    </div>
  `;
